...
@bot.on(zelda_cmd(outgoing=True, pattern=r"sleep ([0-9]+)$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"shutdown$"))
async def killdabot(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**ZELDA USERBOT** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await event.edit("`ZELDA USERBOT Berhasil di matikan!`")
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"restart$"))
async def killdabot(event):
    await event.edit("**ZELDA USERBOT Berhasil di Restart**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**ZELDA USERBOT Berhasil Di Restart**"
        )
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"readme$"))
async def reedme(e):
    await e.edit(
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"repeat (.*)"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"repo$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"string$"))
async def repo_is_here(wannasee):
    """For .repo command, just returns the repo URL."""
    await wannasee.edit(
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"raw$"))
async def raw(event):
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"reverse(?: |$)(\d*)"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"send (.*)"))

...
@bot.on(zelda_cmd(outgoing=True, pattern=r"song (.*)"))
async def _(event):
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"vsong(?: |$)(.*)"))
async def _(event):
    reply = await event.get_reply_message()
    if event.pattern_match.group(1):
        query = event.pattern_match.group(1)
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"smd (?:(now)|(.*) - (.*))"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"net (?:(now)|(.*) - (.*))"))
async def _(event):
    if event.fwd_from:
        return
    if event.pattern_match.group(1) == "now":
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"mhb(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(
    zelda_cmd(outgoing=True, pattern=r"deez (.+?|) (FLAC|MP3\_320|MP3\_256|MP3\_128)")
)
async def _(event):

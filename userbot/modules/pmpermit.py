...
@bot.on(zelda_cmd(outgoing=True, pattern=r"notifoff$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"notifon$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
async def approvepm(apprvpm):
    ...
    await apprvpm.edit(f"Halo [{name0}](tg://user?id={uid}) pesan kamu telah diterima!")
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"(?:tolak|nopm)\s?(.)?"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"block$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"unblock$"))
...
@bot.on(zelda_cmd(outgoing=True, pattern=r"(set|get|reset) pm_msg(?: |$)(\w*)"))
